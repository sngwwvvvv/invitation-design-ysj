...
const checks = [
  ...
  ["reduces invitation text by exactly 1px", /\.invitation-message\s*\{[^}]*font-size:\s*calc\(1rem\s*\+\s*1px\)/.test(mobileBlock?.[1] ?? "")],
  ["reduces career-list text by exactly 1px", /\.career-list\s*\{[^}]*font-size:\s*calc\(\.9375rem\s*\+\s*1px\)/.test(mobileBlock?.[1] ?? "")],
